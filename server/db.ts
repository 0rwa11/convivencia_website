...
import { InsertUser, users, evaluations, sessions, InsertEvaluation, InsertSession } from "../drizzle/schema";
...
export async function getEvaluations() {
  ...
  return await db.select().from(evaluations).orderBy(evaluations.date);
}
...
export async function updateEvaluation(id: number, data: Partial<InsertEvaluation>) {
  ...
  return await db.update(evaluations).set(data).where(eq(evaluations.id, id));
}
...
export async function deleteEvaluation(id: number) {
  ...
  return await db.delete(evaluations).where(eq(evaluations.id, id));
}
